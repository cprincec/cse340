...
switch ($action) {
    case 'upload':
        ...
        if ($imageCheck) {
            $message = '<p class="notice">An image by that name already exists. <span class="close-notice">X</span></p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p class="notice">You must select a vehicle and image file for the vehicle.<span class="close-notice">X</span></p>';
        } else {
            ...
            if ($result) {
                $message = '<p class="notice">The upload succeeded. <span class="close-notice">X</span></p>';
            } else {
                $message = '<p class="notice">Sorry, the upload failed. <span class="close-notice">X</span></p>';
            }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p class='notice'>$filename was successfully deleted. <span class='close-notice'>X</span></p>";
        } else {
            $message = "<p class='notice'>$filename was NOT deleted.<span class='close-notice'>X</span></p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = '<p class="notice">Sorry, no images could be found.<span class="close-notice">X</span></p>';
        }
