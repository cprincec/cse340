...
function getThumbnailByVehicleId($vehicleId) {
    $db = phpmotorsConnect();
    $sql = "SELECT images.imgPath, inventory.invMake, inventory.invModel
    FROM images 
    INNER JOIN inventory
    ON images.invId = inventory.invId
    WHERE  images.invId = :vehicleId AND imgPath LIKE '%-tn.%'";
    $stmt = $db->prepare($sql);
